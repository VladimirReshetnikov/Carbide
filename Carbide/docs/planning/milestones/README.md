...
- **M8 — Webcil mode. Implemented; no separate plan document.** `<WasmEnableWebcil>true</WasmEnableWebcil>`, so managed assemblies ship as `.wasm` and no `.dll` content type is served. The delivered shape differs from the architecture plan's §7 sketch in three ways, each recorded in that section's *Implementation outcome* subsection and in the [drift notes](../../drift/README.md): Webcil replaces the PE headers rather than wrapping a PE (so Carbide reads the container and builds references from the raw metadata root); dual-form ref packs were deliberately not built because no host consumes them; and T3's forked `System.Console` overlay now targets the `.wasm` asset, relying on Mono dispatching by image magic rather than by extension.
