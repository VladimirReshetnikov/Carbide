...
- Implemented today:
  ...
  - Roslyn source generators and diagnostic analyzers, registered via `session.addAnalyzer` /
    `project.addAnalyzer`, the CLI's `--analyzer` flag, `<Analyzer Include>` items and
    `<ProjectReference OutputItemType="Analyzer">` in a `.csproj`, or the `analyzers/` assets
    of a resolved NuGet package; generated source reaches diagnostics, the emitted assembly,
    and execution, and an analyzer rule at error severity fails a build
- Deliberately not implemented yet:
  - `.sln` parsing (vision §7 caps shape S5 at one-plus-siblings)
  - code-fix providers — Carbide has no editor to apply a fix in; a DLL carrying only those is
    refused by `addAnalyzer` with a message naming what it found, rather than accepted and
    ignored
...
| Capture-bypass reporting | Supported (P0.1) | `Console.OpenStandardOutput()` / `OpenStandardError()` call sites are reported as `MSCAP001` and `Console.OpenStandardInput()` as `MSCAP002` in `RunResult.diagnostics`; the CLI routes them into `warnings`. Detection is Roslyn symbol-bound. The escaped bytes themselves are still not captured — the runtime's fd layer is not a supported extension point |
