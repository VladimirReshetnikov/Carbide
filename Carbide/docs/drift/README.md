...
- **`build()` uses `DynamicallyLinkedLibrary` output kind, `run()` uses `ConsoleApplication`.** A library without a `Main` builds cleanly through `project.build()`; the same source would fail under `project.run()` with CS5001. Callers who want strict entry-point checking should prefer `run()` or implement their own check. [M4 plan `ProjectCompiler.BuildAsync` / `RunAsync`.]
